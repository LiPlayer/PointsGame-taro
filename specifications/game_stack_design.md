...
-   **颜色控制系统 (Color Systems)**:
    ...
    -   **3. 动态渐变背景系统 (Background Lightness System)**:
        ...
        -   **亮度偏移**: $\Delta L_{time} = 20 \times \sin(\frac{2\pi \times t}{60})$。
        -   **背景定值**:
            -   **顶部 (Top)**: $25 + \Delta L_{time}$ (在 5 ~ 45 之间循环)。
            -   **底部 (Bottom)**: $10 + \Delta L_{time}$ (在 0 ~ 30 之间循环)。
-   **光影配置 (Lighting)**:
    -   **禁用物理光源**: 禁用所有物理光源 (DirectionalLight, AmbientLight 等)。
    -   **纯色赋值**: 场景不依赖引擎光影计算，直接将 2.2 章节定义的 HSL 值赋值给材质，通过亮度分层模拟光影，以获得最纯净的色彩表现和最高性能。
