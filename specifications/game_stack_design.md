...
-   **3D 等距视角 (Isometric View)**:
    -   取消透视畸变，确保视觉一致性。
    -   摄像机沿 Y 轴平滑跟随堆叠高度上移。
-   **底部淡出 (Bottom Fade)**: 塔基底部体积雾，形成淡化效果。

-   **颜色控制系统 (Color Systems)**:
    -   **1. 全局色相系统 (Global Hue System)**:
        -   **共享变量 (Shared H)**: 全场景（方块与背景）共享唯一色相变量 $H$。
        -   **初始色相 ($H_0$)**: 随机生成 (0-359)。
        -   **步进规则**: 每生成一个新方块，$H = (H + 5) \pmod{360}$。色相是画面唯一的动态驱动变量。
    -   **2. 饱和度系统 (Saturation System)**:
        -   **方块饱和度 ($S_{Block}$)**: **80** (保持高鲜艳度、实体感)。
        -   **背景饱和度 ($S_{BG}$)**: **30** (确保环境退后、不抢夺视线)。
    -   **3. 动态渐变背景系统 (Background Lightness System)**:
        -   **垂直线性渐变**: 顶部始终比底部亮 15 个单位。
        -   **呼吸循环**: 随游戏运行时间 $t$ 进行 60 秒周期的亮度起伏。
        -   **亮度偏移**: $\Delta L_{time} = 30 \times \cos(\frac{2\pi \times t}{60})$ (幅度 60%)。
        -   **背景定值**:
            -   **顶部 (Top)**: $50 + \Delta L_{time}$ (在 20 ~ 80 之间循环)。
            -   **底部 (Bottom)**: $40 + \Delta L_{time}$ (在 10 ~ 70 之间循环)。
-   **光影配置 (Lighting)**:
    -   **物理光源**: 启用物理光照以获得真实的立体感。
        -   **主光源 (Key Light)**: DirectionalLight, 强度 0.8, 从左上方照射 ((-100, 200, 100))，产生投影。
        -   **环境光 (Fill Light)**: AmbientLight, 强度 0.6, 提供基础照明。
    -   **材质**: 使用 `MeshStandardMaterial`，配合光照产生自然的明暗面。

### 2.2 方块与物理 (Blocks & Physics)
-   **多色调方块 (Multi-Tone Blocks)**:
    -   **亮度规律 (Pseudo-Lighting)**: 通过固定亮度阶梯模拟左上方平行光源。
        -   **顶面 (Top)**: `hsl(H, 80%, 65%)` (受光面，最亮)。
        -   **左面 (Left)**: `hsl(H, 80%, 50%)` (固有色面)。
        -   **右面 (Right)**: `hsl(H, 80%, 35%)` (阴影面，最暗)。
