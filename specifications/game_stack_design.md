...
-   **目标**: 堆叠无限高的方块塔。
-   **操作**: 全屏任意位置单点触摸 (Tap)。
-   **方块初始尺寸**: 正方形，如 `100 × 100` 单位。塔基与第一个移动方块同尺寸。
-   **流程**:
    1.  **待机 (Idle)**: 显示单块塔基，屏幕提示 "点击开始"。
    2.  **游戏 (Playing)**: 新方块从屏幕外一侧滑入，沿 X/Z 轴交替往复移动。往复范围约为塔宽度的 **1.5-2 倍**，滑出可视范围后立刻折返。点击屏幕锁定位置。
...
-   **完美对齐 (Perfect Stack)**:
    ...
    -   **视觉**: 方块自身 **短暂闪白 (Block Flash)**，连续 Perfect 时闪烁 **逐渐变亮** (combo 越高越亮)。辅以白色波纹扩散 (Ripple Effect)。
    -   **听觉**: 音调提升 (Pitch Up)。
-   **回血机制 (Growth/Heal)**:
    -   **触发**: 每次 Perfect 时，方块即刻 **微量增大**（约 +2-3%）。
    -   **效果**: 连续 Perfect 产生持续增长效果，可叠加直至恢复初始尺寸。增长即时生效，给玩家"即时奖励感"。
    -   **重置**: 任何一次非 Perfect 放置，增长链断裂，方块尺寸仅由切割决定。
-   **速度曲线 (Difficulty)**:
    -   初始速度设为 `V`。
    -   **阶梯式加速 (Step Function)**: 每 **15-20 块** 速度跳增一档（约 +15-20%）。
    -   速度在约 **50-80 分** 后封顶/稳定，让玩家重新找到节奏。
    -   上限约为 **2.5V**。
...
-   **3D 等距视角 (Isometric View)**:
    -   取消透视畸变，确保视觉一致性。
    -   摄像机沿 Y 轴平滑跟随堆叠高度上移。
    -   **底部淡出 (Bottom Fade)**: 画面始终聚焦塔顶约 5-8 层，下方旧方块逐渐淡出或被推出视野，保持画面简洁。
-   **颜色系统 (Color System)**:
    -   **初始色相**: 每局游戏开始时，从 0-360° **随机** 选取一个起始色相 (startHue)。塔基颜色 = HSL(startHue, S, L)。
    -   **色相旋转 (Hue Rotation)**: 每成功放置 1 个方块，色相 (Hue) 偏移约 **3-5度**，在色相环上缓慢均匀旋转。
    -   **色彩参数**: 饱和度 55-65%、明度 60-70%，维持柔和、温暖的高级感。避免过高饱和度导致的刺眼感。
    -   **背景**: 使用方块当前色的 **高明度 (85-90%)、低饱和度 (30-40%) 版本**，作为纯色或轻微 **线性渐变 (Linear Gradient)** 背景。背景色随方块色调平滑过渡。
-   **光影配置**:
    ...
    -   **环境光**: `AmbientLight` (0.8强度，提升整体明亮度)。

### 2.2 方块与物理 (Blocks & Physics)
-   **双色方块 (Dual-Tone Blocks)**:
    -   **顶面 (Top Face)**: 使用主色的浅色版（亮度 +10-15%）。
    -   **侧面 (Side Faces)**: 使用主色的深色版（亮度 -5-10%，饱和度 +5%）。
    -   这是原版最标志性的视觉特征，需要在材质层面定义两种颜色。
...
### 2.3 视觉特效 (VFX)
-   **完美闪白 (Perfect Block Flash)**: 触发 Perfect 时，方块自身 **叠加白色**。连续 combo 时闪烁亮度逐级增强，给玩家"快要触发奖励"的期待感。这是最主要的 Perfect 反馈。
-   **完美波纹 (Perfect Ripple)**: 触发 Perfect 时，白色线框从中心向外扩散（次要效果）。
