...
-   **流程**:
    ...
    2.  **游戏 (Playing)**: 新方块从屏幕外一侧滑入，沿 X/Z 轴交替往复移动。往复范围约为塔宽度的 **1.2倍**，滑出可视范围后立刻折返。点击屏幕锁定位置。
    3.  **切割 (Slicing)**: 未重合部分被物理切断掉落。上方新方块继承切割后的尺寸。
    4.  **结束 (Game Over)**: 当方块完全未重合时，**仅最后一块方块从塔上滑落/翻倒**。
        -   **宏观回看**: 摄像机平滑缩放至全景，展示整座塔，保持塔基位于屏幕底部。
        -   **分数动画**: 当前分数滑入屏幕中央放大展示。
        -   **交互**: 点击屏幕任意位置退出到结算页。
...
-   **颜色控制系统 (Color Systems)**:
    -   **1. 全局色相系统 (Global Hue System)**:
        ...
        -   **初始色相 ($H_0$)**: 随机生成 (0-359)。
        -   **步进规则**: 每生成一个新方块，$H_{target} = H_{start} + 12 \times Score$。
...
-   **背景粒子 (Background Particles)**: 
    -   **视觉表现**: 场景中漂浮着 60 个随机分布的半透明白色微尘粒子 (`Points`)。
    -   **运动逻辑**: 粒子持续缓慢向上漂浮，到达边界后循环回到底部。
    -   **视差效果 (Parallax)**: 粒子系统的 Y 轴位置随摄像机移动，但仅跟随 80% 的位移量，营造深度的空间感。
...
-   **展示**:
    -   **宏观回看**: 摄像机自动缩放至全景 (Overview)，展示完整作品。
    -   **分数展示**: 分数放大并移动至屏幕中央。
-   **页面跳转**:
    -   **手动退出**: 点击屏幕任意位置跳转至 **Result Page**。
    -   **无自动退出**: 保持展示状态直到用户操作。
