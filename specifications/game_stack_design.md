...
-   **速度曲线 (Difficulty)**:
    -   初始速度设为 `V`。
    -   **动态调整**:
        -   **完美对齐 (Perfect)**: 速度保持不变 (稳定节奏)。
        -   **非完美对齐 (Non-Perfect)**: 速度增加 **1%** (微惩罚)。
        -   **音阶顶点 (Scale Peak)**: 每达到 **8连击** (High C Note)，速度增加 **3%** (进阶挑战)。
    -   **上限**: 速度在 **2.5V** 封顶。
...
-   **背景粒子 (Background Particles)**: 
    -   **分层设计**: 粒子系统分为背景层 (`bgParticles`) 和前景层 (`fgParticles`)，以增强 3D 深度感。
    -   **背景层**: 位于方块后方 (X/Z < -1)，透明度较低 (0.3)，移动速度较慢。
    -   **前景层**: 位于方块前方 (X/Z > 1)，透明度较高 (0.5)，移动速度较快。
    -   **运动逻辑**: 粒子持续缓慢向上漂浮，到达边界后循环回到底部。
    -   **视差效果 (Parallax)**: 粒子系统的 Y 轴位置随摄像机移动，但仅跟随 80% 的位移量。
...
| **Perfect Combo** | 音高逐级上升，使用 **C大调音阶 (C Major)**：Do-Re-Mi... | 每次 + 一度，**每 8 次循环** | 悦耳、明亮 |
| **回血触发** | 当连续 8+ Perfect 时，叠加一个 **和弦重音** | 主音 + 五度 | 饱满、奖励感 |
| **切割 (Slice)** | 极短的 "咔嗒" | ~300Hz | 干、硬 |
| **Game Over 坠落** | 低沉的 "咚" + 短暂混响 | ~80-100Hz | 重低音 Thud |

### 3.2 音效行为规则
-   **每次放置都有声音**: 无论 Perfect 与否，点击放置的瞬间都必须有声音反馈。
-   **Perfect 音高递增**: 使用 8 音阶循环 (C5 -> C6)。每 8 个 combo 完成一个八度循环。
